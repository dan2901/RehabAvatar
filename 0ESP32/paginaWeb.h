//pageContent variable is of type char, now we dont need to escape double quotes by backslash \ 
//you can code the whole webpage in any editor, then paste the code here
//PROGMEM - instructs Arduino to store the variable into flash memory (program memroy) rather than SRAM
...
<html lang="en">
  <head>
    ...
    <title>WebPageRehabAvatar</title>
    <style>
      body {
      ...
      .parentDiv {
        display: flex; /*flexible box layout --> makes it easier to arrange elements*/
        width: 100%;
        margin-bottom: 20px;
      }
      ...
      .angle-circle {
        ...
        background: conic-gradient(
          #4ca431 0deg,
          #ffffff 0deg
        ); /* initial angle, that has 0 of green, and the rest is just white */
      ...
      .angle-text {
        /* the angles on the cirlce, for reference purposes */
        position: absolute;
        color: #000000;
        font-weight: bold;
        font-size: 14px;
      ...
      .center-angle {
        /* angle performed by the accelerometer */
        font-size: 36px;
        font-weight: bold;
        color: #005744;
        z-index: 10;
  ...
  <body>
    <div class="header">
      <h2 style="margin: 0">RehabAvatar - Gait Analysis</h2>
    </div>
    <div class="parentDiv">
      <div class="childDiv">
        <p>
          For the game to work, you need to follow
          <a href="https://172.20.10.2">this link</a>
          and allow the access: Advanced -> Proceed Anyway
        </p>
      </div>
    </div>
    ...
    <div class="game-container">
      ...
      <div class="circle-container">
        ...
        <div class="angle-circle" id="angleCircle">
          ...
          >
            <!-- top: 0% vertical, bottom: 100% vertical, left: 0% horizontal, right: 100% horizontal; "translate" moves the element with 50% of its width (fine adjustments)  -->
            90°
          </div>
          <div
            class="angle-text"
    ...
    <h2>Analysing your gait parameters</h2>

    <div class="parentDiv">
      <div class="childDiv">
        <div>
          <h4>
            Your leg's acceleration <br />
            in the direction of walking (m/s<sup>2</sup>)
          </h4>
    ...
    <script>
      ...
      //function to add values to graphs
      ...
      //gets non secure data from ESP32
      ...
      websock.onmessage = function (event) {
        ...
        //low pass filter
        //the filtered value has more influence on the final reading
        let a = 0.3;
        filteredAccel = a * netAccel + (1 - a) * filteredAccel;

        if (Math.abs(filteredAccel) < 0.05) filteredAccel = 0; //removes noise, by not reading small values
